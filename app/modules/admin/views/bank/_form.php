...
<div class="bank-form form">
    ...
    <?= $form->field($model, 'nama_payment')->textInput(['maxlength' => true]) ?>
    <?= $form->field($model, 'biaya_per_transaksi')->textInput(['maxlength' => true]) ?>
    <?= $form->field($model, 'is_active')->dropDownlist($model::getIsActive(), ['prompt' => '']) ?>
    <?= $form->field($model, 'is_va')->dropDownlist($model::getIsVA(), ['prompt' => '']) ?>
    <?= $form->field($model, 'va_number')->textarea(['placeholder' => 'Using Format JSON, please see doc midtrans', 'maxlength' => true]) ?>
    <a href="https://snap-docs.midtrans.com/#request-body-json-parameter" target="_blank">See Doc Midtrans</a><br>
